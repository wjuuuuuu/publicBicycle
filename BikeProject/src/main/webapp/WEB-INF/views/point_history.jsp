...
<h4>마일리지포인트:<span id = "mPoint"></span></h4>
<div id = "schBox">
<div id = "dateContent">
...
<c:forEach var = "points" items = "${point}">
<div id = "data">
    
    <div id = "data1">${points.rentalNo}</div>
    <div id = "data2">${points.time}</div>
    <div id = "data3"><fmt:formatDate value="${points.rentalTime}"/></div>
</div>
 </c:forEach>
<div id = "paging"></div>
<div class="pagination_section">
<c:if test="${paging.startPage != 1 }">
			<a href="/BikeProject/point.do?id=${RentalVo.id}&nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
		</c:if>
		<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
			<c:choose>
				...
				<c:when test="${p != paging.nowPage }">
					<a href="/BikeProject/point.do?id=${RentalVo.id}&nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
				</c:when>
			</c:choose>
		</c:forEach>
		<c:if test="${paging.endPage != paging.lastPage}">
			<a href="/BikeProject/point.do?id=${RentalVo.id}&nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}">&gt;</a>
		</c:if>
</div>
...
$(document).ready(function(){
	...
	$("#search").click(function(){
		...
		emailform1($("#toDate").val(),$("#fromDate").val(),id);
	});
});
...
function emailform1(toDate,fromDate,id){
	var frm1 = document.createElement('form');
	frm1.setAttribute('method', 'post');
	frm1.setAttribute('action', '/BikeProject/point.do');
	var hiddenInput1 = document.createElement('input');
	hiddenInput1.setAttribute('type','hidden');
	hiddenInput1.setAttribute('name','fromDate');
	hiddenInput1.setAttribute('value',toDate);
	frm1.appendChild(hiddenInput1);
	var hiddenInput2 = document.createElement('input');
	hiddenInput2.setAttribute('type','hidden');
	hiddenInput2.setAttribute('name','toDate');
	hiddenInput2.setAttribute('value',fromDate);
	frm1.appendChild(hiddenInput2);
	var hiddenInput3 = document.createElement('input');
	hiddenInput3.setAttribute('type','hidden');
	hiddenInput3.setAttribute('name','id');
	hiddenInput3.setAttribute('value',id);
	frm1.appendChild(hiddenInput3);
...
function totalPoint(){
	$.ajax({
		...
		success: function(data){
            ...
            $("h4>#mPoint").text(data.totalPoint);
        },
	})
}
